# -*- coding: utf-8 -*-
"""COVID_vs NonCOVID data.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/17msfdnQ5OEOrANd8-KGXaqQNosiqXErd
"""

from google.colab import drive
drive.mount('/content/gdrive')

import pandas as pd
import re
import string
import numpy as np
import nltk
import json
import glob
import gzip
import os
import random

covid_files_path = '/content/gdrive/My Drive/twitter data/Twitter_Covid_Data/*.json.gz'
covid_files = glob.glob(covid_files_path)

train_count = 0
train_data = []
train_labels = []

print(covid_files)

# lengths = []
# for i in range(len(covid_files)):
#     print("opening file", covid_files[i])
#     try:
#         with gzip.open(covid_files[i],'rb',) as fin:
#           tweets = fin.read().splitlines()
#           lengths.append(len(tweets))


#     except:
#         print("Faulty file ", covid_files[i])

# print(sum(lengths)/len(lengths))

tweet_ids = set()
counter = 0
with open('/content/gdrive/My Drive/twitter data/covid_tweets.json', 'a') as l:
  for i in range(len(covid_files)):
      print("opening file", covid_files[i])
      try:
          with gzip.open(covid_files[i],'rb',) as fin:
            j = 0
            tweets = fin.read().splitlines()
            while j<= 100:
              covid_tweets_train = {}
              random_tweet = random.choice(tweets)
              tweet = json.loads(random_tweet)
              try:
                # Filtering Retweets  
                while (tweet['retweeted_status']): 
                  random_tweet = random.choice(tweets)
                  tweet = json.loads(random_tweet)
                
              except:
                #Filtering replies, language, and duplicates
                if tweet['in_reply_to_user_id'] == None and tweet['lang'] == 'en' and tweet['id'] not in tweet_ids:
                  j += 1             
                  tweet_ids.add(tweet['id'])
                  covid_tweets_train['id'] = tweet['id']

                  if tweet['truncated'] == True:
                    covid_tweets_train['text'] = tweet['extended_tweet']['full_text']
                  else:
                    covid_tweets_train['text'] = tweet['text']

                  json.dump(covid_tweets_train,l)
                  l.write('\n')
                  #covid_tweets_train.clear()
                  #print(i, folder)
      except:
          print("Faulty file ", covid_files[i])
          counter += 1

print(counter)

count = 0
with open('/content/gdrive/My Drive/twitter data/covid_tweets.json', 'r') as l:
  for line in l:
    count += 1

print(count)